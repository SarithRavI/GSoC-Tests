...
def read_graph_data(path):
    ...
    images_raw = dataset["X"]
    labels = dataset["y"]
    
    num_node_attr = 6  # = num of channels(4) + x,y coord
    num_edge_attr =1 
    ...
    for img_inx,img in enumerate(tqdm(images_raw)):
        # expand dimention in first coord 
        img = np.expand_dims(img,axis=0) # now the image's shape is 1,134,4 
        # convert the images into np arrays of shape (4,1,134)
        ...
        # here global positions are in 2D dim
        # first components is x (width dim)
        # second component is y (height dim)
        global_locs = np.vstack((mask_coord[1],mask_coord[0])).T
        # get vertices for the graph  
        # vertices shape is (4, # num of nodes)
        ...
        # if we set to 2 it will yiels just a sequense 
        # we set k=4 for much better structure
        n_neighbors = min(4,vertices.shape[0]-1)
        adj = kneighbors_graph(vertices, n_neighbors, mode='connectivity', include_self=False)
        vertices_dist = kneighbors_graph(vertices, n_neighbors, mode='distance', include_self=False)
...
class QGJetsGraphsDataset(InMemoryDataset):
    ...
    def __init__(self,root,name,transform=None, pre_transform=None,pre_filter=None):
        ...
        super(QGJetsGraphsDataset, self).__init__(self.root, transform, pre_transform)
    ...
    def processed_file_names(self):
        if self.pre_transform is not None:
            return f"geometric_QGJets_processed_{repr(self.pre_transform)[:-2]}.pt"
        return 'geometric_QGJets_processed.pt'

    def process(self):
        raw_file = [file for file in self.raw_file_names][0]
